...
| ID | Requisito original resumido | Status | Evidência | Lacuna | Critério de aceite | Prioridade | Dependências | Risco |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PROC-01 | Validar tipos e schema drift. | `partially_validated` | `contracts.py`, `diff.py`, `normalization.py`; drift completo no staging e drift isolado entre duas fontes no PostgreSQL local | Contratos por fonte ainda não foram exercitados por um runtime persistente. | Cenários públicos continuam cobertos e qualquer drift bloqueante gera evidência sem alterar outra fonte. | crítica | Fase 3 para validação integrada | R-07 |
| PROC-02 | Tratar exceções sem quebrar a execução total. | `validated` | lote sequencial continua após falha; PostgreSQL local comprovou rollback/retry de A com B intacta; resumo agregado classifica sucesso, falha, busy e inactive | Persistência do resumo depende do futuro runtime, que não existe. | Falha simulada de uma fonte não impede a seguinte e o resultado agregado permanece sanitizado. | crítica | Fases 1–3 | R-08 |
