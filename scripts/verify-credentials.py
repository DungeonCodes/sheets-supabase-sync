...
p=argparse.ArgumentParser();p.add_argument('--format',choices=('text','json'),default='text');p.add_argument('--supabase-only',action='store_true');p.add_argument('--google-only',action='store_true');a=p.parse_args();c=[]
...
try:
 ...
 if not a.supabase_only:
  ...
  add('google_service_account','warning','token_nao_testado_sem_driver')
