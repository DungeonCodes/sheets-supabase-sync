...
class RetentionControlsMigrationTests(unittest.TestCase):
    ...
    def test_purge_runs_store_only_aggregate_evidence(self) -> None:
        ...
        self.assertIn("jsonb_typeof(candidate_counts) = 'object'", self.sql)
        self.assertIn("not dry_run or affected_counts = '{}'::jsonb", self.sql)
        for forbidden in ("payload_json", "row_key_hash", "connection_string", "error_message"):
    ...
    def test_has_no_destructive_dml_or_secret(self) -> None:
        for token in ("delete from", "truncate ", "drop table", "drop column"):
